...
APK_DIR="$PROJECT_ROOT/client/build/app/outputs/flutter-apk"

# Prefer project-named files, fall back to Flutter defaults
if [ -f "$APK_DIR/${APP_NAME}-release.apk" ]; then
    APK_PATH="$APK_DIR/${APP_NAME}-release.apk"
elif [ -f "$APK_DIR/${APP_NAME}-debug.apk" ]; then
    APK_PATH="$APK_DIR/${APP_NAME}-debug.apk"
