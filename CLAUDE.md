...
- **Mobile**: Capacitor 8 (Android + iOS shells around the same bundle; local builds only, no CI)
- **Tests**: Vitest
...
Mobile (Capacitor) — see `web/MOBILE.md`. Builds run locally only; do **not** add
a CI workflow for them (store signing material stays off the repo):
```bash
cd web
npm run build:mobile     # CAPACITOR=1 BASE_PATH=/ — no /Chess/ base, PWA disabled
npm run android:build    # debug APK  -> web/dist-mobile/android/
npm run android:release  # release AAB (signed when ANDROID_KEYSTORE_* is set)
npm run ios:build        # simulator build (macOS only)
npm run ios:release      # .xcarchive + .ipa (macOS only)
npm run mobile:assets    # regenerate native icons + splash screens
```
`web/android/` and `web/ios/` are generated by `npx cap add` on first build and are
gitignored — never hand-edit them. Native settings belong in `web/capacitor.config.ts`,
build logic in `web/scripts/mobile/*.sh`. App version/build number derive from
