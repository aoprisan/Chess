...
Built with **React + Vite + vite-plugin-pwa**, and packaged for the Android and
iOS app stores with **Capacitor** (see [MOBILE.md](MOBILE.md)).
...
The same bundle ships as a native app through Capacitor — built locally, never
in CI (store signing material stays off the repo). Full guide in
[MOBILE.md](MOBILE.md).

```bash
npm run android:build     # debug APK   -> dist-mobile/android/
npm run android:release   # release AAB (signed when the keystore env is set)
npm run ios:build         # simulator build (macOS)  -> dist-mobile/ios/
npm run ios:release       # .xcarchive + .ipa (macOS)
```

`android/` and `ios/` are generated by `npx cap add` on first build and are not
in git; `capacitor.config.ts` and `scripts/mobile/*.sh` are the sources of
truth.
...
resources/           native icon/splash sources (npm run mobile:assets)
scripts/
  ...
  generate-mobile-assets.mjs  native icons + splash screens (npm run mobile:assets)
