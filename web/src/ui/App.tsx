...
import { BACKGROUND_GROUPS, preloadGroups } from './preload';
import { AssetGate, BootScreen } from './AssetGate';
...
export function App() {
  ...
  // Boot blocks only on the menu's art; the heavy groups (heroes, combat,
  // adventure) stream in the background and each screen is wrapped in an
  // <AssetGate>, so no screen ever paints with half-loaded images (the
  // adventure map used to swap biome backgrounds mid-scroll).
  ...
  useEffect(() => {
    let stale = false;
    void preloadGroups(['menu'], (fraction) => {
  ...
  // Once the menu is up, quietly fetch the rest in the order the player is
  // likely to need it. Gates cover the case where they get there first.
  useEffect(() => {
    if (!assetsReady) return;
    void (async () => {
      for (const group of BACKGROUND_GROUPS) await preloadGroups([group]);
    })();
  }, [assetsReady]);
  ...
  if (!assetsReady) {
    return <BootScreen progress={assetProgress} />;
  }
  ...
  if (view.name === 'battle') {
    return (
      <div className="app">
        <AssetGate groups={['heroes', 'combat']}>
          <Combat
  ...
  if (view.name === 'adventure') {
    ...
    return (
      <div className="app">
        <AssetGate groups={['heroes', 'adventure']}>
          <AdventureMap
